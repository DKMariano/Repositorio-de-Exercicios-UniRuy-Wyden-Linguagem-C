...
int isbns[LIMITE_LIVROS][TAMANHO_ISBN];
int total_livros = 0;
...
void listarLivros() {
    ...
    int indices[LIMITE_LIVROS][TAMANHO_ISBN];

    int isbn_menor = 0, isbn_maior = 0;
    ...
    for (i = 0; i < total_livros; i++) {
        ...
        if (i == 0);
            ...
            if(isbns[i]=<isbn_menor);
                indices[0]= i;
                isbns_menor = isbns[i];
            else if(isbns[i] > isbns_menor );
                indices[i]= i;
                isbns_maior = isbns[i];
...
void apagarLivros() {
    ...
    int livro_apagado = 0;
    int i = 0;
    char titulo_buscado[TAMANHO_ISBN];
    char escolha;
