...
/* Substituindo strcspn por uma função auxiliar */
...
void cadastrarLivros() {
    ...
    if (total_livros >= LIMITE_LIVROS) {
        ...
        titulos[total_livros][find_newline(titulos[total_livros])] = '\0'; // Usando função auxiliar

        printf("Nome do autor: ");
        fgets(autores[total_livros], TAMANHO_AUTOR, stdin);
        autores[total_livros][find_newline(autores[total_livros])] = '\0'; // Usando função auxiliar

        printf("ISBN: ");
        fgets(isbns[total_livros], TAMANHO_ISBN, stdin);
        isbns[total_livros][find_newline(isbns[total_livros])] = '\0'; // Usando função auxiliar
...
void buscarLivros() {
    ...
    titulo_procurado[find_newline(titulo_procurado)] = '\0'; // Usando função auxiliar
...
void editarLivros() {
    ...
    livro_editado[find_newline(livro_editado)] = '\0'; // Usando função auxiliar
    ...
    if (indice == -1) {
        ...
        titulos[indice][find_newline(titulos[indice])] = '\0'; // Usando função auxiliar

        printf("Digite o novo autor do livro: ");
        fgets(autores[indice], sizeof(autores[indice]), stdin);
        autores[indice][find_newline(autores[indice])] = '\0'; // Usando função auxiliar

        printf("Digite o novo ISBN do livro: ");
        fgets(isbns[indice], sizeof(isbns[indice]), stdin);
        isbns[indice][find_newline(isbns[indice])] = '\0'; // Usando função auxiliar
...
void apagarLivros() {
    ...
    isbn_buscado[find_newline(isbn_buscado)] = '\0'; // Usando função auxiliar

    {
        int i; // Declaração de variável movida para o início do bloco
        for (i = 0; i < total_livros; i++) {
            if (strcmp(isbns[i], isbn_buscado) == 0) {
                int j; // Declaração de variável movida para o início do bloco
